...
setwd("/Users/lorenzobenedetti/Documents/GitHub/Benedetti-2026-N2O-AgroscenaNext/Rstudio/Graph N2O")
...
prepare_data <- function(data,
  ...
  df_filt <- data %>%
    filter(
      Compound == "N2O",
      !Date %in% as.Date(exclude_dates),
      Type %in% c(treatments, "CTR")
    )
  ...
  ctr_plot <- CTR_by_date %>%
    mutate(
      mean = CTR_Date * 1000 * conv_factor,
...
final_plot <- ggplot(
  ...
  ## SD ribbon (non per CTR)
  geom_ribbon(
    data = subset(plot_data, Treatment != "CTR"),
    aes(
      ymin = lower,
      ymax = upper
    ),
